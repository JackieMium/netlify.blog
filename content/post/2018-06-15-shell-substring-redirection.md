...
再有，在 `command > file 2>&1` 这个命令里， `&` 并不是后台或者 AND 的意思。放在 `>` 后面的 `&`，表示重定向的目标不是一个普通文件，而是一个 `文件描述符`，是标准输入输出这些。所以 `2> 1` 代表将 `STDERR` 即错误输出重定向到当前路径下文件名为 `1` 的 `普通文件` 中，而 `2> &1` 却是代表将重定向到标准输出。而由于标准输出已经被重定向到 `file` 中，因此最终的结果为标准输出和错误输出都被重定向到 `file` 中。
